...
class MSTW_BB_BRACKET_BUILDER {
		...
		<tbody>
		  ...
		  for ( $i = 1; $i <= $rounds; $i++ ) {
			...
			for ( $j = 1; $j <= $round_nbr_games; $j++ ) {
				...
				$this -> build_table_row( $tourney, null, $games, $i, $j + $round_start_game );
			}
	...
	function build_table_row( $tourney, $teams, $games, $round_nbr, $game_nbr ) {
		...
		<tr> <!-- game <?php echo $game_nbr ?> -->
		  ...
		  $this -> build_team_cell( $game, $game_nbr, 'home', null ); 
		  $this -> build_team_text_cell( $game, $game_nbr, 'home' );
		  
		  $this -> build_team_cell( $game, $game_nbr, 'away', null );
	...
	function create_game ( $tourney_slug, $game_nbr, $rnd_nbr, $bracket=1 ) {
		...
		if ( null === $tourney_obj ) {
			mstw_bb_log_msg( "create_game: Oops! No CPT found for slug: $tourney_slug" );
			return -1;
		}
		...
		if( has_action('save_post_mstw_bb_game') ) {
			mstw_bb_log_msg( "create_game: Have a save_post_mstw_bb_game action. Hummm ..." );
			return 0;

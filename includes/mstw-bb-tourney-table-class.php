...
class MSTW_TOURNEY_TABLE {
	...
	function build_tourney_table( $atts ) {
		...
		if ( !array_key_exists( 'tourney', $atts ) ) {
			$html .= __( 'Error: TOURNEY not specified in shortcode.', 'mstw-bracket-builder' );
			return $html;
		}
		...
		if ( null === $tourney_obj ){
			$html .= sprintf( __( 'Error occured with tourney: %s', 'mstw-bracket-builder' ), $tourney_slug );
			return $html;
		}
		...
		//
		// If there are no games for the tourney, return a warning string
		//
		if ( !$games ) {
			$ret_html .= sprintf( __( 'No games found for tourney: %s', 'mstw-bracket-builder' ), $tourney_slug );
			return $ret_html;
		}
		
		//
		// Finally, build the table HTML
		//
	...
	function build_date_cell( $game_nbr, $game, $date_format = 'D, j M Y' ) {
		...
		$ret_html .= date( $date_format, $timestamp );
	...
	function build_location_cell( $game_nbr, $game, $location_format ) {
		...
		$ret_html = '<td class="game_location">';
